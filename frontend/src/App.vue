...
<template>
  ...
  <div v-if="shell" class="app-layout">
    <aside class="sidebar" :class="{ 'is-open': menuOpen }">
      <RouterLink class="brand" to="/invoices"><span class="brand-mark" aria-hidden="true">A<span></span></span><span>{{ t('brand') }}<small>{{ t('workspace') }}</small></span></RouterLink>
      <nav :aria-label="t('menu')"><p class="nav-caption">{{ t('salesSection') }}</p><RouterLink v-for="item in ['invoices', 'customers', 'products']" :key="item" :to="`/${item}`"><Icon :name="item" /><span>{{ t(item) }}</span></RouterLink><p class="nav-caption finance-caption">{{ t('financeSection') }}</p><RouterLink to="/accounting"><Icon name="accounting" /><span>{{ t('accounting') }}</span></RouterLink><RouterLink to="/settings"><Icon name="settings" /><span>{{ t('settings') }}</span></RouterLink></nav>
