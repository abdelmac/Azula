...
const dateFrom = ref(''); const dateTo = ref(''); const customer = ref<number | null>(null); const paymentStatus = ref('')
watch(() => route.query, query => { status.value = ['draft', 'validated'].includes(String(query.status)) ? String(query.status) : ''; paymentStatus.value = ['unpaid', 'overdue', 'settled'].includes(String(query.payment_status)) ? String(query.payment_status) : '' }, { immediate: true })
const filters = computed(() => ({ status: status.value, ordering: ordering.value, customer: String(customer.value ?? ''), date_from: dateFrom.value, date_to: dateTo.value, payment_status: paymentStatus.value }))
...
<template>
  ...
  <section class="card"><div class="table-toolbar"><div class="segmented" role="group" :aria-label="t('status')"><button v-for="value in ['', 'draft', 'validated']" :key="value" :class="{ selected: status === value }" :aria-pressed="status === value" @click="status = value">{{ t(value || 'all') }}</button></div><div class="toolbar-filters"><label class="search-field"><Icon name="search" /><input v-model="search" type="search" :placeholder="t('search')" :aria-label="t('search')" /></label><select v-model="ordering" :aria-label="t('sort')"><option value="-issue_date">{{ t('newest') }}</option><option value="issue_date">{{ t('oldest') }}</option></select></div></div>
    <div class="invoice-date-filters"><label class="field"><span>{{ t('start') }}</span><input v-model="dateFrom" type="date" :max="dateTo || undefined" /></label><label class="field"><span>{{ t('end') }}</span><input v-model="dateTo" type="date" :min="dateFrom || undefined" /></label><SearchSelect v-model="customer" kind="customers" :label="t('customer')" /><label class="field"><span>{{ t('paymentStatus') }}</span><select v-model="paymentStatus" :aria-label="t('paymentStatus')"><option value="">{{ t('all') }}</option><option value="unpaid">{{ t('unpaid') }}</option><option value="overdue">{{ t('overdue') }}</option><option value="settled">{{ t('settled') }}</option></select></label></div>
